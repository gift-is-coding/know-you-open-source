# Onboarding 7 天历史生成设计

## 目标

首次 onboarding 完成前，明确告诉用户 KnowYou 会在本机生成最近 7 天日记，过程可能需要几分钟，并强调数据不上传 KnowYou 后端。用户确认后，系统串行生成 7 个自然日，并在主界面展示按天进度。
...
- 主窗口顶部 notice 显示首次 7 天生成说明与当前按天进度。
- 左侧 My Diary 展示 7 天占位日期；未生成完成的日期正文为空态 `Generating your diary...`。

## 行为规则

- “过去 7 天”包含今天，共 7 个自然日。
...
- onboarding bootstrap day keys 为今天起倒推 7 天。
