...
**Goal:** Add a native Todo inbox that deduplicates daily diary to-do candidates, supports manual promotion, and conservatively auto-completes open items from later evidence.

**Architecture:** Keep daily diaries as narrative artifacts and persist task state in SQLite. Add a small todo domain/store layer, deterministic candidate parsing and matching helpers, optional LLM-backed reconciliation/completion prompts, then wire a sidebar Todo page and diary-row actions into `AppState`.
...
- [ ] Add a `TodoInboxView` with open items first and completed items last.
