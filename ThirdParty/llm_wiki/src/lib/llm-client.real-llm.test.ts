 ...
 * server and assert the request lands with our explicit
 * `Origin: http://localhost` header (NOT a webview-injected
...
describe("streamChat against a fake Ollama server (real TCP)", () => {
  it(
    ...
    async () => {
      ...
      try {
        // Ollama's default OLLAMA_ORIGINS includes http://localhost
        // and http://localhost:* even when the server is bound to
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // to customEndpoint. It still uses the same local-LLM CORS
        // workaround: a fixed Origin accepted by default Ollama setups.
        server.setRejectMode({ kind: "accept-same-origin", selfOrigin: "http://localhost" })
