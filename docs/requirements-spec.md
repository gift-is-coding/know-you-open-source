...
- `Networking` activation 必须使用 App 本机机器账号连接 Supabase，而不是依赖 Web 手动注册或无状态匿名占位；activation state 必须本地保存 `authEmail`、`authPassword`、`refreshToken`、profile server mapping 和 plaintext agent token，并且 plaintext agent token 不得出现在 MCP tool 输出或 Web URL query 中。旧 `localSandbox` 或缺少机器账号凭证的平台 state 不得被视为 ready，必须触发可重试的重新 activation。
...
- `Networking` App 的 `Open Square` 只能在真实平台已连接、profile 已批准且已同步后可用；打开 Web 时必须使用 fragment handoff 传递 Supabase session 和 platform id，access/refresh token 不得进入 query string。未配置 Web base URL 的 release build 必须显示 inline error，不得把 token-bearing handoff 指向 localhost；localhost 默认只允许 DEBUG 开发构建。
...
- 首次 onboarding 的固定顺序必须为：`demoRead`、`demoClick`、`demoReference`、`privacy`、`installApp`、`permissions`、`enginePrompt`、`engineSetup`、`generating`
...
- My Wiki 读取层必须固定读取 llm_wiki 原生 `Sources`、`Entities`、`Concepts` 三类，不得再从 `mywiki.schema.json` 或旧 schema config 派生分类
- 默认首页索引必须按 `Entities`、`Concepts`、`Sources` 顺序显示；每个分类展开时默认最多显示 10 个 name-only 条目，不显示 summary 或 category badge
- 当前分类如果存在 frontmatter `tags`，必须提供动态 tag 筛选；tag facet 必须来自当前条目统计，按出现频次降序，同频按名称排序。不得写死 `人物`、`项目`、`组织`、`其他`，也不得把 tag 重新变成独立 ontology 目录
- `Recent`、`Needs Review` 等必须作为 view 处理，不得作为 ontology category 生成对应 wiki 目录
- 每个原生分类分组必须支持展开/折叠；超过 10 个条目时必须在当前分类底部提供 `Show more (N)` 原地展开全部，并提供 `Show less` 收回。不得用 `View all` 或分类全量列表页作为默认展开方式
- 用户可见分类必须是 `Sources`、`Entities`、`Concepts`；不得保留 People/Projects/Topics/Patterns/Follow-ups 等旧分类 UI，也不得继续读取旧 `wiki/people`、`wiki/projects`、`wiki/topics`、`wiki/preferences`、`wiki/follow-ups` 等目录作为兼容入口
...
- 系统必须能创建 My Wiki 项目结构，包括 `raw/sources/`、`wiki/sources/`、`wiki/entities/`、`wiki/concepts/`；不得创建 `purpose.md`、`mywiki.schema.json`、`schema.md`，也不得创建 People/Projects/Topics/Preferences/Follow-ups/Summaries 等旧目录
- 系统必须能把已有 `YYYY-MM-DD.md` 日记通过 Source Catalog materialization 写入稳定 `raw/sources` 路径，并保留原文 Markdown；重复 materialize 同一 source 必须覆盖稳定文件名，不得生成重复文件
...
- 默认 My Wiki pipeline 每次运行最多处理 3 个 manifest source；没有 manifest 且限定批量时，headless runner 必须优先选择还没有 `wiki/sources/<source>.md` 的最新 raw source；旧生成内容清理后重跑也必须按小批次逐步推进
- LLM Wiki headless runner 收到 manifest 时必须只处理 manifest 内列出的 project-relative `raw/sources` 路径，并把 source 的 `folderContext` 通过原生 `autoIngest` 参数传入；它不得通过扫描整个 `raw/sources` 目录决定本次 eligible source set
- LLM Wiki headless runner 不得生成 KnowYou 自定义 output contract、schema guide、purpose prompt 或 entity tag prompt；entity/concept 生成和合并必须完全沿用 llm_wiki 原生 `autoIngest` prompt 与 pipeline
