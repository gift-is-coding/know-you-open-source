...
- Home 下方四个入口单列展示，顺序为 `Networking (Coming soon)`、`Today’s Diary`、`My Wiki`、`Add Sources`，每项用短但完整的人话解释用途。
...
- Sidebar root 顺序保持 `Home`、`Networking (Coming soon)`、`Todo`、`My Wiki`、`Other Source`、`My Diary`。
- Diary 列表窗口最多只含 today 加前三天，demo day 仍按 onboarding 规则附加。
- Home copy 为英文，包含本地自动更新时间、`Generate Now`、条件化的 `Generate Last 3 Days` 和四个单列功能入口。
