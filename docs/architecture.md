...
Networking V1 只保留两个 KnowYou 自有平台：`knowyou-jobs`（Know You 求职 / Know You Careers）和 `knowyou-friends`（认识新朋友 / Find Your Friends）。每个平台绑定一个已确认 profile；`NetworkingPlatformConfiguration.canRunAutomation` 同时要求平台 active、profile 已生成、profile 已批准。社区和消息合并在 `Communities and messages` 区域：点击社区后，下方 matched profile、approval 状态、agent status、消息、入站、出站、agent activity 与 highlight 都按 `NetworkingCockpitItem.platformID` 过滤。Cockpit 顶部有三步 guidance strip（Generate profile、Approve & sync、Open your square），空 inbox 文案指向当前未完成步骤。`NetworkingActivationRunner` 使用 App 本机生成的机器账号走 Supabase signup/password sign-in；若旧 activation state 保存了 `authEmail` 与 `authPassword`，runner 会直接 password sign-in 做幂等恢复，随后 upsert person/profile、激活 community membership、注册只保存 hash 的 agent token，并把 `authEmail`、`authPassword`、`refreshToken`、profile server mapping 和 agent token plaintext 只持久化到本地 `.knowyou/networking/activation.json`。只有真实平台模式且机器账号凭证完整的 activation state 才能被视为 ready；旧 local sandbox 或缺凭证状态会落入重新 activation。缺少真实 Supabase 配置时 cockpit 明确显示配置失败，不再写占位 activation。profile approval state 额外保存 local profile 到 server profile 的 sync record，供 App 重启后判断 `Open Square` 是否可用。`Open Square` 使用单一 `NetworkingBackendConfiguration` 生成 handoff URL；localhost Web 默认只存在于 DEBUG 构建，release 缺少 Web base URL 时显示按钮旁 inline error。`NetworkingInboxService` 合并本地 `networking_record_highlight` inbox state 与平台 Agent Home 队列，供 cockpit 展示真实 inbound/highlight/outbound 条目。`KnowYou --networking-mcp --project-root <path>` 暴露 `networking_publish_post`、`networking_publish_comment`、`networking_agent_home`、`networking_record_decision`、`networking_fetch_public_square`、`networking_record_highlight`；未开启、local sandbox、不完整平台凭证或 token 缺失时写入 tool 返回 permission required，真实平台模式下 public square fetch 读 Supabase public posts，highlight 写入本地 cockpit inbox state，MCP 输出不回显 plaintext token。
