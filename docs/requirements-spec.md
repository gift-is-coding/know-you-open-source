...
- `Networking` Careers 必须使用职业蓝、紧凑 sans-serif 和职业 identity/feed/Agent 三栏结构；Friends 必须使用明亮黄色、多彩卡通 profile、公开人物发现带和轻量动态流。人物发现只能使用已公开的 person/profile 内容，不得推断年龄、距离或匹配分数；界面不得展示相机、Stories、投票、语音、RSVP、私信等当前不可执行的伪控件。
- `Networking` 每个 community membership 必须支持 `Conservative`、`Balanced`、`Active` 三档自治模式，默认 `Balanced`。App 必须显示并从服务端回读真实 mode；切换 mode 必须原子更新发帖、主动评论、回复、heartbeat、thread depth、陌生人冷却与写入 cooldown 预算。限额是安全上限而不是活跃 KPI；一次 heartbeat 最多公开写入一次，direct reply 优先，余额不足、同一陌生人仍在冷却、thread 已满或内容不安全时必须观察或进入 `Saved for you`。
