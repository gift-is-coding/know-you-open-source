...
struct MyWikiContextPackRequest: Equatable {
    let projectRoot: URL
    let query: String
    let maxItems: Int
    let characterBudget: Int

    init(projectRoot: URL, query: String, maxItems: Int = 8, characterBudget: Int = 6_000) {
...
struct MyWikiContextPack: Codable, Equatable {
    let query: String
    let queryPlan: MyWikiContextQueryPlan
    let items: [MyWikiContextItem]
    let citations: [MyWikiContextCitation]
}

struct MyWikiContextQueryPlan: Codable, Equatable {
    let phrases: [String]
    let terms: [String]
}

struct MyWikiContextItem: Codable, Equatable {
    let title: String
    let pageType: String
    let excerpt: String
    let score: Int
    let matchedTerms: [String]
    let citation: MyWikiContextCitation
}

struct MyWikiContextCitation: Codable, Equatable {
    let relativePath: String
    let sourceKind: String
    let day: String?
}

struct MyWikiContextPackService {
    var fileManager: FileManager = .default

    func contextPack(for request: MyWikiContextPackRequest) -> MyWikiContextPack {
        let queryPlan = Self.queryPlan(for: request.query)
        guard request.maxItems > 0,
              request.characterBudget > 0,
              queryPlan.isEmpty == false,
              fileManager.fileExists(atPath: request.projectRoot.path)
        else {
            return MyWikiContextPack(query: request.query, queryPlan: queryPlan, items: [], citations: [])
        }

        let candidates = markdownFiles(in: request.projectRoot)
            .compactMap { page(from: $0, projectRoot: request.projectRoot) }
            .compactMap { candidate(for: $0, queryPlan: queryPlan) }
            .sorted { lhs, rhs in
                if lhs.item.score != rhs.item.score {
                    return lhs.item.score > rhs.item.score
                }
                return lhs.item.citation.relativePath < rhs.item.citation.relativePath
            }

        var remainingBudget = request.characterBudget
        var items: [MyWikiContextItem] = []

        for candidate in candidates {
            guard items.count < request.maxItems, remainingBudget > 0 else { break }
            let excerpt = Self.trim(candidate.item.excerpt, maxCharacters: min(candidate.item.excerpt.count, remainingBudget))
            guard excerpt.isEmpty == false else { continue }
            items.append(
                MyWikiContextItem(
                    title: candidate.item.title,
                    pageType: candidate.item.pageType,
                    excerpt: excerpt,
                    score: candidate.item.score,
                    matchedTerms: candidate.item.matchedTerms,
    ...
    private func page(from url: URL, projectRoot: URL) -> MyWikiContextPage? {
        ...
        let sourceKind = relativePath.hasPrefix("wiki/") ? "wiki" : "raw-source"
        let day = parsed.frontmatter["day"] ?? Self.dayFromPath(relativePath)

        return MyWikiContextPage(
            title: parsed.title,
            pageType: parsed.frontmatter["type"] ?? sourceKind,
            body: parsed.body,
    ...
    private func candidate(
        for page: MyWikiContextPage,
        queryPlan: MyWikiContextQueryPlan
    ) -> MyWikiContextCandidate? {
        ...
        for phrase in queryPlan.phrases {
            let phraseScore = scoreTerm(phrase, title: title, path: path, type: type, body: body, titleWeight: 130, pathWeight: 70, typeWeight: 35, bodyWeight: 24)
            if phraseScore > 0 {
                score += phraseScore
        ...
        for term in queryPlan.terms {
            let termScore = scoreTerm(term, title: title, path: path, type: type, body: body, titleWeight: 80, pathWeight: 40, typeWeight: 20, bodyWeight: 10)
            if termScore > 0 {
                score += termScore
        ...
        score += page.sourceKind == "wiki" ? 50 : 0
        if page.relativePath == "wiki/index.md" || page.relativePath == "wiki/overview.md" {
            score -= 35
        }

        let excerpt = Self.excerpt(from: page.body, queryPlan: queryPlan)
        guard excerpt.isEmpty == false else { return nil }

        return MyWikiContextCandidate(
            item: MyWikiContextItem(
                title: page.title,
                pageType: page.pageType,
                excerpt: excerpt,
                score: score,
                matchedTerms: Self.uniquePreservingOrder(matchedTerms),
                citation: MyWikiContextCitation(
    ...
    private static func parseMarkdown(
        ...
        if markdown.hasPrefix("---\n"), let endRange = markdown.range(of: "\n---", range: markdown.index(markdown.startIndex, offsetBy: 4)..<markdown.endIndex) {
            ...
            let bodyStart = markdown.index(endRange.upperBound, offsetBy: markdown[endRange.upperBound...].first == "\n" ? 1 : 0, limitedBy: markdown.endIndex) ?? endRange.upperBound
    ...
    private static func queryPlan(for background: String) -> MyWikiContextQueryPlan {
        let runs = tokenRuns(in: background)
        var terms: [String] = []
        ...
        var phrases = quotedPhrases(in: background)
    ...
    private static func tokenRuns(in text: String) -> [MyWikiContextTokenRun] {
        var runs: [MyWikiContextTokenRun] = []
        var latin = ""
        var cjk = ""

        func flushLatin() {
            if latin.isEmpty == false {
                runs.append(MyWikiContextTokenRun(kind: .latin, text: latin))
                latin = ""
            }
        }
        ...
        func flushCJK() {
            if cjk.isEmpty == false {
                runs.append(MyWikiContextTokenRun(kind: .cjk, text: cjk))
                cjk = ""
            }
        }
    ...
    private static func excerpt(from body: String, queryPlan: MyWikiContextQueryPlan) -> String {
        let paragraphs = body
...
private extension MyWikiContextQueryPlan {
    var isEmpty: Bool {
        phrases.isEmpty && terms.isEmpty
    }
}

private enum MyWikiContextTokenKind {
    case latin
    case cjk
}

private struct MyWikiContextTokenRun {
    let kind: MyWikiContextTokenKind
    let text: String
}

private struct MyWikiContextPage {
    let title: String
    let pageType: String
    let body: String
    let relativePath: String
...
private struct MyWikiContextCandidate {
    let item: MyWikiContextItem
}
