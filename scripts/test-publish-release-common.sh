...
appcast_xml="$(release_update_appcast_xml "sparkle:edSignature=\"abc123\" length=\"123456\"")"
assert_contains "$appcast_xml" '<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">' "appcast namespace"
assert_contains "$appcast_xml" '<sparkle:version>116</sparkle:version>' "appcast build version"
...
KNOWYOU_RELEASE_NOTES_FILE="$custom_notes_file" custom_appcast_xml="$(release_update_appcast_xml "sparkle:edSignature=\"abc123\" length=\"123456\"")"
assert_contains "$custom_appcast_xml" 'Custom release notes' "custom appcast release notes"
assert_contains "$custom_appcast_xml" '- Better updates' "custom appcast bullet"
