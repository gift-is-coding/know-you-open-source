...
function PresetRow({
  ...
  // CLI providers authenticate via the user's existing local OAuth
  // state, so no API key field is shown. Ollama ditto for local-only.
  const needsApiKey =
        ...
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          {state === "ok" && (
            <>
              ...
              <div className="text-muted-foreground">
                If ingest fails with an authentication error, open Codex or run{" "}
                <code className="rounded bg-background/60 px-1 py-0.5 font-mono text-[10px]">
                  codex
                </code>{" "}
                in a terminal to refresh the local login.
              </div>
            </>
          )}
          {state === "err" && (
            <>
              ...
              <div className="text-muted-foreground">
                Open the Codex desktop app or install the CLI, then re-check.
              </div>
            </>
          )}
        </div>
